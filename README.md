...
## Lógica del chatbot
Este es un agente que utiliza a Gemini (aunque este se puede cambiar fácilmente en el backend.py) para el razonamiento del chatbot; se le da una base de datos con los datos recolectados de todos los municipios de Antioquia. Los datos recolectados para cada documento son los siguientes:
...
* Capacidad hospitalaria en IPS

Basado en estos datos, el chatbot razonará una respuesta a cualquier pregunta respecto a Antioquia.
...
Instaladas las dependencias, se debe añadir una API key para poder acceder al modelo; esto se hace modificando el backend.py en la línea 11.
```
genai.configure(api_key="")#Pongan la API key, recomiendo gemini porque es gratis
```

Luego, si se necesita, se puede modificar el modelo usado en la línea 49
```
model = genai.GenerativeModel(
    model_name="gemini-3-flash-preview",
    ...
```
Luego de esto se ejecuta el código y se corre el archivo index.html.
